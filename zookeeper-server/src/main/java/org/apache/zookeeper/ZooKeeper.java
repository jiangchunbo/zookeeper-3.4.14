...
public class ZooKeeper {
    ...
    public ZooKeeper(String connectString, int sessionTimeout, Watcher watcher,
        ...
        // connectStringParser 解析器已经解析好了字符串，通过 getServerAddresses 可以获取到底有多少个主机
        HostProvider hostProvider = new StaticHostProvider(connectStringParser.getServerAddresses());

        // 创建 ClientConnection 客户端连接，然后 start 启动里面的线程
     ...
     *                       "/foo/bar" would result in operations being run on
     *                       "/app/a/foo/bar" (from the server perspective).
     *                       <p>以逗号分隔的 host:port 对列表，表示 ZooKeeper 服务器地址。例如 127.0.0.1:3000,127.0.0.1:3001
     *                       <p>如果使用 chroot 后缀，也就是改变根路径 (例如 127.0.0.1:3000/app)，所有路径将基于 /app/ 根目录
     * @param sessionTimeout session timeout in milliseconds
     *                       <p>会话超时时间，单位：毫秒
     * @param watcher        a watcher object which will be notified of state changes, may
     *                       also be notified for node events
     *                       <p>一个监听器 Watch 对象，用于接收状态变化或节点事件的通知
     * @param sessionId      specific session id to use if reconnecting
     *                       <p>重连时使用的特定会话 ID (如果时新会话不需要提供)
     * @param sessionPasswd  password for this session
     *                       <p>重连时使用的会话密码
     * @param canBeReadOnly  (added in 3.4) whether the created client is allowed to go to
     *                       read-only mode in case of partitioning. Read-only mode
    ...
    private static ClientCnxnSocket getClientCnxnSocket() throws IOException {
        // zookeeper.clientCnxnSocket -> 该系统属性可以修改使用的网络框架
