...
public class ClientCnxn {
     ...
     * <p>
     * 已经发送给服务端，等待结果的 Package。
     * <p>
     * 而且 Package 被用于 synchronized 等待/唤醒
     */
    ...
    public ClientCnxn(String chrootPath, HostProvider hostProvider, int sessionTimeout, ZooKeeper zooKeeper,
        ...
        // 连接超时时间，这样可以确保所有的 hostProvider 都能尝试一遍，如果全都失败了，那么也差不多超过了 connectTimeout
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void readResponse(ByteBuffer incomingBuffer) throws IOException {
             ...
             */
            try {
                // 按理说，Client 的 XID 应该等于 Server 的 XID，否则 Server 可能丢掉了我的请求？
                ...
                // 填充 replyHeader (之前 new 好了的 header)
        ...
        public void run() {
            ...
            // 只要不是 close，也不是 auth 失败，就认为活着
            while (state.isAlive()) {
                try {
                    // 首次连接，或者连接关闭了，都走下面的逻辑，就是重新建立
                    if (!clientCnxnSocket.isConnected()) {
                        // 如果不是第一次连接，就睡眠，因为第一次连接需要快速触达一次
                        if (!isFirstConnect) {
                            try {
                    ...
                    if (state.isConnected()) {
                        ...
                        // readTimeout: 表示多久没收到任何数据就判定会话失效(=sessionTimeout * 2/3)
                        // clientCnxnSocket.getIdleSend(): 距离上一次向 Server 发送任何字节已经过去的时间（毫秒）

                        // 推演: sessionTimeout 30s  readTimeout 20s
                        //      readTimeout/2 = 10s，也就是 10 秒必须有一次心跳

                        // 正常情况下，每个 readTimeout/2 主动发送一次 ping
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        // 该方法要返回响应头，因此先 new 出来，然后等里面填充，然后 return
        ReplyHeader r = new ReplyHeader(); // 待填充
        Packet packet = queuePacket(h, r, request, response, null, null, null,
                null, watchRegistration);

        // 等待/唤醒，明确唤醒肯定是其他线程唤醒，不可能是自己唤醒
        // 其他线程是 SendThread，SendThread 在 doIO 时，会拿到 pendingQueue
        synchronized (packet) {
